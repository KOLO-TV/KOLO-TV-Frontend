...
import { Bookmark, ChevronRight, Clock, Pause, Play, Share2, ThumbsUp, Volume2, type LucideIcon } from 'lucide-react'
...
const ACTIONS: { icon: LucideIcon; label: Label }[] = [
  { icon: ThumbsUp, label: { key: 'emission.actions.like', params: { count: '2.4K' } } },
  { icon: Share2, label: { key: 'emission.actions.share' } },
  { icon: Bookmark, label: { key: 'emission.actions.save' } },
]

const TAGS: TranslationKey[] = [
  'emission.tags.politics',
  'emission.tags.antananarivo',
  'emission.tags.economy',
  'emission.tags.barea',
  'emission.tags.culture',
]
    ...
    <SiteLayout>
      <div className="mx-auto max-w-7xl px-4 py-6 lg:px-8 lg:py-10">
        <nav className="mb-4 flex items-center gap-1.5 text-xs text-slate-500">
          ...
          <span>{t('emission.breadcrumbShows')}</span>
          <ChevronRight className="h-3 w-3" />
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-slate-500">
                <span className="flex items-center gap-1">
                  <Clock className="h-4 w-4" /> {t('emission.duration', { count: 28 })}
                </span>
                <span>·</span>
                <span>{t('time.todayEvening')}</span>
                <span>·</span>
                <span>{t('emission.views', { count: '124 512' })}</span>
              </div>
              ...
              <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
                <h2 className="font-display text-lg font-bold">{t('emission.aboutTitle')}</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  {t('emission.aboutText')}
                </p>
            ...
            <div className="mt-12">
              <h2 className="mb-6 font-display text-2xl font-black">{t('emission.related')}</h2>
          ...
          <aside className="lg:sticky lg:top-24 lg:h-fit">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
              <div className="mb-3 flex items-center justify-between">
                <h2 className="font-display text-lg font-black">{t('emission.recentEpisodes')}</h2>
                <span className="text-xs text-slate-500">{t('emission.episodesCount', { shown: 6, total: 240 })}</span>
              </div>
                  ...
                  <li key={ep.titleKey}>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 text-xs text-slate-500">{t(ep.date.key, ep.date.params)}</div>
                      </div>
                    </button>
                  </li>
                ))}
